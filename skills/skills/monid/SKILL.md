...
Everything below is local setup, not part of the upstream skill. The rest of this
file is fetched verbatim from https://monid.ai/SKILL.md and gets replaced wholesale
on update, so keep local notes in this section only.

**Already done.** CLI installed globally (`npm i -g @monid-ai/cli@latest`), `monid setup
--client claude-code` run, key added as label `main` and activated. Verified with
`monid balance`. Do not repeat the setup flow, and never ask for an API key.

**Where the secret lives.** Two copies, neither in git:

| Location | What it is |
|---|---|
| macOS Keychain, service `monid-api-key`, account `monid` | The canonical copy. Survives a CLI reinstall or a `~/.config/monid` wipe. |
| `~/.config/monid/credentials.yaml` (mode 0600) | The CLI's own store. This is what `monid run` actually reads. |

Read it back without printing it to the transcript:

```bash
security find-generic-password -a monid -s monid-api-key -w
```

Restore the CLI store from the Keychain if it is ever lost:

```bash
monid keys add -k "$(security find-generic-password -a monid -s monid-api-key -w)" -l main
```

The key must never be written into `~/dotfiles`. That repo is git tracked and its
`.gitignore` covers only `.DS_Store`, so a plaintext key there would be committed.

**Updating this skill.** The CLI version and this file's frontmatter `version` must
match. When they drift, bring both to latest rather than downgrading either:

```bash
npm install -g @monid-ai/cli@latest
curl -sL https://monid.ai/SKILL.md -o ~/dotfiles/skills/skills/monid/SKILL.md
```

That overwrites this section, so re-append it afterwards. The symlink at
`~/.claude/skills/monid` points here, so no reinstall step is needed.

**Precedence on this machine.** The `direction-first` rule runs first for anything
that looks like a tool or library recommendation: search BLANK before naming
anything, Monid included. Monid's own precedence rule then applies, so an existing
MCP server or personal API key wins over a Monid endpoint. Monid is the fallback for
gaps, and it spends real balance.
